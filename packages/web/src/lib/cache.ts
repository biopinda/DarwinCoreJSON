...
export async function generateCache(): Promise<TaxonomicCache> {
  try {
    ...
    console.log(
      `🔍 Found ${families.length} families with phenological data:`,
      families.slice(0, 5)
    )
    ...
    console.log(
      `🔄 Processando gêneros para ${cleanFamilies.length} famílias...`
    )
...
export async function getCachedFamilies(): Promise<string[]> {
  try {
    ...
    if (!cache) {
      console.log(
        '🔄 Cache não encontrado ou desatualizado, gerando novo cache...'
      )
      try {
  ...
  try {
    ...
    if (!cache) {
      try {
        ...
        console.warn(
          '⚠️  Falha ao gerar cache, usando fallback para gêneros:',
          cacheError
        )
        // Fallback para busca direta no MongoDB
        const calFeno = await getCollection('dwc2json', 'calFeno')
  ...
  try {
    ...
    if (!cache) {
      try {
        ...
        console.warn(
          '⚠️  Falha ao gerar cache, usando fallback para espécies:',
          cacheError
        )
        // Fallback para busca direta no MongoDB
        const calFeno = await getCollection('dwc2json', 'calFeno')
