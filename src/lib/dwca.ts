  ...
  await streamProcessor(filePath, (line) => {
    ...
    (obj[id][extensionName] as Record<string, unknown>[]).push(
      fields.reduce((acc, field, index) => {
        if (field !== "INDEX" && values[index]) {
          acc[field] = values[index].charAt(0) === "{"
...
export const buildSqlite = async (folder: string, chunkSize = 5000) => {
  {
    ...
    const ref = {
      core: _parseJsonEntry(archive.core),
      extensions:
        (Array.isArray(archive.extension)
          ? archive.extension
          : [archive.extension].filter(Boolean)).map(_parseJsonEntry),
    };
      ...
      *[Symbol.iterator]() {
        ...
        GROUP BY
          ...
          try {
            ...
            batch = rows.map((
              { id, json },
            ) => [id, JSON.parse(json as string)]) as [
              string,
              RU,
            ][];
            yield batch;
...
export const processaEml = (emlJson: Eml): Ipt => {
  const [id, version] = emlJson["@packageId"].match(/(.+)\/(.+)/)?.slice(1) ??
    [];
