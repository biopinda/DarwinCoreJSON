    ...
    Object.entries(dwcJson).reduce((entries, [id, taxon]) => {
      ...
      if (distribution) {
        taxon.distribution = {
          ...
          phytogeographicDomains: distribution[0]?.occurrenceRemarks
      ...
      if (taxon.resourcerelationship) {
        ...
        taxon.othernames = resourcerelationship.map((relationship) => ({
          ...
          scientificName: dwcJson[relationship.relatedResourceID as string]
            ?.scientificName,
      ...
      (
        ...
        entry.vernacularName = entry.vernacularName
          .toLowerCase()
          .replace(/ /g, "-");
        entry.language = entry.language.charAt(0).toUpperCase() +
